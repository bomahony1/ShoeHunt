...
class SavingToPostgresPipeline(object):
    ...
    def process_item(self, item, spider):
        query = """
        INSERT INTO nike (title, category, original_price, discount_price, discount_percent, image_url, description, product_url)
...
class SavingToPostgresPipeline2(object):
    def __init__(self):
        self.create_connection()
    ...
    def create_connection(self):
        ...
        drop_table_sql = "DROP TABLE IF EXISTS jdsport"
        self.curr.execute(drop_table_sql)

        sql = """
        CREATE TABLE IF NOT EXISTS jdsport (
            timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            title VARCHAR(256),
            category VARCHAR(256),
            original_price VARCHAR(256),
    ...
    def process_item(self, item, spider):
        query = """
        INSERT INTO jdsport (title, category, original_price, discount_price, discount_percent, image_url, description, product_url)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """
        data = (
            item["title"],
